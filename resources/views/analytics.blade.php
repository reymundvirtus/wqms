                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_f < 47.00 && data[i].temperature_f > 40.00) {
                            ...
                            html += '<h5 class="mb-0" data-cooltipz-dir="top" data-cooltipz-size="medium" aria-label="Temperature is normal"><small class="badge badge-success">Normal</small></h5><small class="mb-0">Indicator</small>'
                        } else if (data[i].temperature_f > 90.00 && data[i].temperature_f < 100.00) {
                            html += '<h5 class="mb-0" data-cooltipz-dir="top" data-cooltipz-size="large" aria-label="If the water temperature is too hot for tilapia, it can also have negative effects on the fish. High temperatures can lead to stress, which can weaken the fish\'s immune system and make them more susceptible to disease. Additionally, high water temperatures can cause a decrease in oxygen levels in the water, which can lead to suffocation and death. Furthermore, high temperatures can also cause a decrease in the dissolved oxygen levels in the water, leading to a reduced growth rate and suboptimal feeding. Tilapia can tolerate a wide range of temperatures, but temperatures above 92°F (33°C) can be dangerous and will require a cooling system for the fish, such as a chiller, to maintain the optimal temperature."><small class="badge badge-warning">Warm</small></h5><small class="mb-0">Indicator</small>'
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_f < 47.00 && data[i].temperature_f > 40.00) {
                            ...
                            html += '<h5 class="mb-0">Water is normal</h5><small class="mb-0">Rest Assured</small>'
                        } else if (data[i].temperature_f > 90.00 && data[i].temperature_f < 100.00) {
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_f < 47.00 && data[i].temperature_f > 40.00) {
                            ...
                            var details = "Temperature value is "+ data[i].temperature_f +"°F. This means the water is COLD \"nobody wants to be in a cold environment\". Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                                url: '/api/send-notif',
                                success : function() {
                                    return 0;
                                },
                            ...
                            var details = "Temperature value is "+ data[i].temperature_f +"°F. This means the water is TOO COLD \"nobody wants to be in a very cold environment\". Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                            ...
                            var subject = "Temperature Abnormal - Alert level: YELLOW"
                            var details = "Temperature value is "+ data[i].temperature_f +"°F. This means the water is WARM \"nobody wants to be in a hot environment\". Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                            ...
                            var subject = "Temperature Abnormal - Alert level: RED"
                            var details = "Temperature value is "+ data[i].temperature_f +"°F. This means the water is TOO HOT \"nobody wants to be in a very hot environment\". Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_pH < 6.00 && data[i].temperature_pH > 3.00) {
                            html += '<h5 class="mb-0" data-cooltipz-dir="top" data-cooltipz-size="medium" aria-label="Please do action ASAP, fishes might die in this situation"><small class="badge badge-warning">Warm Acid</small></h5><small class="mb-0">Indicator</small>'
                        } else if (data[i].temperature_pH < 3.00) {
                            html += '<h5 class="mb-0" data-cooltipz-dir="top" data-cooltipz-size="medium" aria-label="Please do action ASAP, fishes will SURELY DIE in this situation"><small class="badge badge-danger">Acidic</small></h5><small class="mb-0">Indicator</small>'
                        } else if (data[i].temperature_pH > 6.00) {
                            html += '<h5 class="mb-0" data-cooltipz-dir="top" data-cooltipz-size="large" aria-label="The optimal pH range for tilapia is between 6.5-8.5. However, they can tolerate pH levels as low as 6.0 and as high as 9.0. It\'s important to note that pH levels that are too low or too high can cause stress to the fish and weaken their immune systems, making them more susceptible to disease."><small class="badge badge-success">Normal</small></h5><small class="mb-0">Indicator</small>'
                        }
                    }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_pH < 6.00 && data[i].temperature_pH > 3.00) {
                            var subject = "Temperature Abnormal - Alert level: YELLOW"
                            var details = "pH value is "+ data[i].temperature_pH +". This means the water is warm acid. Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                            ...
                            var subject = "Temperature Abnormal - Alert level: RED"
                            var details = "pH value is "+ data[i].temperature_pH +". This means the water is acidic. Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
        ...
        function currentMoist() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        html += '<h5 class="mb-0">' + data[i].temperature_moist + ' %</h5><small class="mb-0">Current Moisture</small>'
                    }
                ...
                success: function(data) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        if (data[i].temperature_moist < 6.00 && data[i].temperature_moist > 3.00) {
                            ...
                            var details = "Moisture value is "+ data[i].temperature_moist +". This means the rain is moderate. Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
                            ...
                            var details = "Moisture value is "+ data[i].temperature_moist +". This means the rain is heavy. Please check you fishpond, ASAP"
                            $.ajax({
                                type: 'POST',
                                dataType: 'json',
                                data: { subject: subject, details: details },
